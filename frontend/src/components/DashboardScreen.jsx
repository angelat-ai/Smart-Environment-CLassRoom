    ...
    <group position={position}>
      ...
      <mesh position={[0, 0.55, 0]}>
        ...
        <meshStandardMaterial
          color={isOn ? '#fff9c4' : '#d0d0d0'}
          transparent opacity={0.9}
          emissive={isOn ? '#ffeb3b' : '#000'}
...
function SmartInsightsPage({ rooms, selectedRoom }) {
    ...
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        ...
        <button
          ...
          style={{
            ...
            padding: '10px 20px', background: 'var(--dark)', color: 'var(--green)',
            border: '1.5px solid rgba(0,200,150,0.3)', borderRadius: 9,
        ...
        >
          {loading ? (
            <>
              <span style={{ display: 'inline-block', width: 14, height: 14, border: '2px solid rgba(0,200,150,0.3)', borderTop: '2px solid var(--green)', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
              Analyzing…
            </>
          ) : (
            <>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 2l2 7h7l-5.5 4 2 7L12 16l-5.5 4 2-7L3 9h7z"/></svg>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 24 }}>
        {[
          ...
          { label: 'Peso Savings', val: `₱${((room?.energy_saved_today || 0) * MERALCO_RATE).toFixed(2)}`, color: '#f59e0b' },
      ...
      {error && (
        <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 10, padding: '14px 16px', color: '#991b1b', fontSize: '0.85rem', marginBottom: 20 }}>
          {error}
        </div>
      )}
      ...
      {insights.length === 0 && !loading && !error && (
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '48px 32px', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: '0.85rem', color: 'var(--text-3)', maxWidth: 380, margin: '0 auto', lineHeight: 1.7 }}>
            Click "Generate AI Insights" to get personalized energy optimization recommendations based on your actual room data.
          </div>
        </div>
      )}
          ...
          {insights.map((insight, i) => {
            ...
            return (
              <motion.div
                ...
                style={{ background: cfg.bg, border: `1px solid ${cfg.border}`, borderRadius: 12, padding: '20px 22px' }}
              >
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                  <div style={{ width: 32, height: 32, borderRadius: 8, background: cfg.border, display: 'flex', alignItems: 'center', justifyContent: 'center', color: cfg.color, fontSize: '1rem', flexShrink: 0 }}>
                    {cfg.icon}
                  </div>
                  <div>
                    ...
                    <div style={{ display: 'inline-block', background: cfg.border, borderRadius: 6, padding: '3px 10px', fontSize: '0.75rem', fontWeight: 600, color: cfg.color }}>
                      {insight.saving}
                    </div>
                  </div>
                </div>
...
function AutomationPage({ addToast }) {
  ...
  const saveRules = (updated) => {
    setRules(updated)
    localStorage.setItem('sec_rules', JSON.stringify(updated))
  }
    ...
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        ...
        <button
          ...
          style={{
            display: 'flex', alignItems: 'center', gap: 8,
            padding: '9px 18px', background: adding ? 'var(--bg)' : 'var(--dark)',
            color: adding ? 'var(--text-2)' : 'var(--green)',
            border: `1.5px solid ${adding ? 'var(--border)' : 'rgba(0,200,150,0.3)'}`,
            borderRadius: 9, fontFamily: 'Geist, sans-serif', fontSize: '0.83rem',
      ...
      <AnimatePresence>
        {adding && (
          <motion.div
            ...
            style={{ background: 'var(--surface)', border: '1px solid var(--green)', borderRadius: 12, padding: '24px', marginBottom: 20, overflow: 'hidden' }}
          >
            <div style={{ fontWeight: 600, fontSize: '0.9rem', color: 'var(--text-1)', marginBottom: 16 }}>New Automation Rule</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              <div>
                <label style={{ fontSize: '0.75rem', color: 'var(--text-3)', fontWeight: 500, display: 'block', marginBottom: 6 }}>Rule Name</label>
                <input value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} placeholder="e.g. Auto lights off after class" style={{ width: '100%', padding: '9px 12px', border: '1.5px solid var(--border)', borderRadius: 8, fontFamily: 'Geist, sans-serif', fontSize: '0.85rem', color: 'var(--text-1)', background: 'var(--bg)', outline: 'none', boxSizing: 'border-box' }} />
              </div>
              <div>
                <label style={{ fontSize: '0.75rem', color: 'var(--text-3)', fontWeight: 500, display: 'block', marginBottom: 6 }}>Room</label>
                <select value={form.room} onChange={e => setForm(p => ({ ...p, room: e.target.value }))} style={{ width: '100%', padding: '9px 12px', border: '1.5px solid var(--border)', borderRadius: 8, fontFamily: 'Geist, sans-serif', fontSize: '0.85rem', color: 'var(--text-1)', background: 'var(--bg)', outline: 'none' }}>
                  {['A', 'B', 'C'].map(r => <option key={r}>Room {r}</option>)}
                </select>
              </div>
              <div>
                <label style={{ fontSize: '0.75rem', color: 'var(--text-3)', fontWeight: 500, display: 'block', marginBottom: 6 }}>Trigger (IF)</label>
                <select value={form.trigger} onChange={e => setForm(p => ({ ...p, trigger: e.target.value }))} style={{ width: '100%', padding: '9px 12px', border: '1.5px solid var(--border)', borderRadius: 8, fontFamily: 'Geist, sans-serif', fontSize: '0.85rem', color: 'var(--text-1)', background: 'var(--bg)', outline: 'none' }}>
                  {Object.entries(triggerLabels).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
                </select>
              </div>
              <div>
                <label style={{ fontSize: '0.75rem', color: 'var(--text-3)', fontWeight: 500, display: 'block', marginBottom: 6 }}>Action (THEN)</label>
                <select value={form.action} onChange={e => setForm(p => ({ ...p, action: e.target.value }))} style={{ width: '100%', padding: '9px 12px', border: '1.5px solid var(--border)', borderRadius: 8, fontFamily: 'Geist, sans-serif', fontSize: '0.85rem', color: 'var(--text-1)', background: 'var(--bg)', outline: 'none' }}>
                  {Object.entries(actionLabels).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
                </select>
              </div>
              <div>
                <label style={{ fontSize: '0.75rem', color: 'var(--text-3)', fontWeight: 500, display: 'block', marginBottom: 6 }}>Delay (minutes)</label>
                <input type="number" min={1} max={60} value={form.delay} onChange={e => setForm(p => ({ ...p, delay: parseInt(e.target.value) || 5 }))} style={{ width: '100%', padding: '9px 12px', border: '1.5px solid var(--border)', borderRadius: 8, fontFamily: 'Geist, sans-serif', fontSize: '0.85rem', color: 'var(--text-1)', background: 'var(--bg)', outline: 'none', boxSizing: 'border-box' }} />
              </div>
              <div style={{ display: 'flex', alignItems: 'flex-end' }}>
                <button onClick={addRule} style={{ width: '100%', padding: '10px', background: 'var(--green)', color: 'white', border: 'none', borderRadius: 9, fontFamily: 'Geist, sans-serif', fontSize: '0.88rem', fontWeight: 500, cursor: 'pointer' }}>
                  Save Rule
                </button>
              </div>
            </div>
            <div style={{ marginTop: 14, padding: '10px 14px', background: 'rgba(0,200,150,0.06)', borderRadius: 8, fontSize: '0.82rem', color: 'var(--text-2)' }}>
              <strong style={{ color: 'var(--text-1)' }}>Preview: </strong>
              IF <strong>{triggerLabels[form.trigger]}</strong> for <strong>{form.delay} min</strong> in <strong>Room {form.room}</strong> → THEN <strong>{actionLabels[form.action]}</strong>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {rules.length === 0 ? (
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '48px 32px', textAlign: 'center' }}>
          ...
          <div style={{ fontFamily: "'Instrument Serif', serif", fontSize: '1.3rem', color: 'var(--text-1)', marginBottom: 8 }}>No Rules Yet</div>
          <div style={{ fontSize: '0.85rem', color: 'var(--text-3)', lineHeight: 1.7 }}>Create automation rules to automatically control lights based on motion, time, or occupancy.</div>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {rules.map((rule, i) => (
            <motion.div
              ...
              style={{ background: 'var(--surface)', border: `1.5px solid ${rule.enabled ? 'rgba(0,200,150,0.2)' : 'var(--border)'}`, borderRadius: 12, padding: '18px 20px', display: 'flex', alignItems: 'center', gap: 16, opacity: rule.enabled ? 1 : 0.6 }}
            >
              <div onClick={() => toggleRule(rule.id)} style={{ width: 38, height: 22, borderRadius: 11, cursor: 'pointer', flexShrink: 0, background: rule.enabled ? 'var(--green)' : 'var(--border)', position: 'relative', transition: 'background 0.2s' }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 600, fontSize: '0.9rem', color: 'var(--text-1)', marginBottom: 4 }}>{rule.name}</div>
                <div style={{ fontSize: '0.8rem', color: 'var(--text-3)' }}>
                  IF <span style={{ color: 'var(--text-2)', fontWeight: 500 }}>{triggerLabels[rule.trigger]}</span> for {rule.delay}m in Room {rule.room}
                  {' → '}<span style={{ color: 'var(--green)', fontWeight: 500 }}>{actionLabels[rule.action]}</span>
                </div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: '0.7rem', fontWeight: 600, padding: '3px 9px', borderRadius: 99, background: rule.enabled ? 'rgba(0,200,150,0.1)' : 'rgba(0,0,0,0.05)', color: rule.enabled ? 'var(--green)' : 'var(--text-3)' }}>
                  {rule.enabled ? 'Active' : 'Paused'}
                </span>
                <button onClick={() => deleteRule(rule.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-3)', padding: '4px', borderRadius: 6 }}>
...
function SimulationPage({ rooms, selectedRoom, loadRooms, addToast }) {
  ...
  const handleOn = async () => {
    setSimulating(true)
    try { await simulateMotion(selectedRoom); await loadRooms(); addToast('Lights ON — motion simulated!', 'success') }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
      <div style={{ marginBottom: 24 }}>
        <h2 style={{ fontFamily: "'Instrument Serif', serif", fontSize: '1.5rem', color: 'var(--text-1)', fontWeight: 400 }}>Simulation Mode</h2>
        <p style={{ fontSize: '0.85rem', color: 'var(--text-3)', marginTop: 4 }}>Test your system behavior without physical hardware</p>
      </div>
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '24px', marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <button onClick={handleOff} disabled={simulatingOff || scenarioRunning} style={{ padding: '11px 28px', background: 'none', border: '1.5px solid var(--red)', borderRadius: 9, color: 'var(--red)', fontFamily: 'Geist, sans-serif', fontSize: '0.88rem', fontWeight: 500, cursor: 'pointer', transition: 'all 0.18s' }} onMouseEnter={e => { e.target.style.background = 'var(--red)'; e.target.style.color = 'white' }} onMouseLeave={e => { e.target.style.background = 'none'; e.target.style.color = 'var(--red)' }}>
            {simulatingOff ? 'Turning off…' : '🌑 Lights OFF'}
          </button>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginLeft: 8 }}>
            <div style={{ width: 10, height: 10, borderRadius: '50%', background: room?.is_active ? 'var(--green)' : '#ccc', animation: room?.is_active ? 'livePulse 1.8s infinite' : 'none' }} />
            <span style={{ fontSize: '0.82rem', color: 'var(--text-2)' }}>Current: <strong>{room?.is_active ? 'ON' : 'OFF'}</strong></span>
          </div>
        </div>
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14 }}>
          {scenarios.map(sc => (
            <div key={sc.id} style={{ background: scenario === sc.id ? 'rgba(0,200,150,0.06)' : 'var(--bg)', border: `1.5px solid ${scenario === sc.id ? 'rgba(0,200,150,0.3)' : 'var(--border)'}`, borderRadius: 10, padding: '18px' }}>
              ...
              <button onClick={() => runScenario(sc)} disabled={scenarioRunning} style={{ width: '100%', padding: '8px', background: scenario === sc.id && scenarioRunning ? 'var(--green-dim)' : 'var(--dark)', color: 'var(--green)', border: '1px solid rgba(0,200,150,0.2)', borderRadius: 7, fontFamily: 'Geist, sans-serif', fontSize: '0.8rem', fontWeight: 500, cursor: scenarioRunning ? 'not-allowed' : 'pointer', opacity: scenarioRunning && scenario !== sc.id ? 0.5 : 1 }}>
                {scenario === sc.id && scenarioRunning ? '▶ Running…' : '▶ Run'}
              </button>
            </div>
          ))}
        </div>
        ...
        {scenarioRunning && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} style={{ background: 'var(--dark)', borderRadius: 12, padding: '20px 24px', marginTop: 0, overflow: 'hidden' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              <span style={{ color: 'var(--green)', fontFamily: 'monospace', fontSize: '0.85rem', fontWeight: 600 }}>● Scenario Running</span>
              <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.78rem' }}>{scenarioProgress}%</span>
            </div>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ fontSize: '0.85rem', color: 'var(--text-3)', marginTop: 4 }}>Compare energy usage and performance across all rooms</p>
      </div>
        ...
        {rooms.map((room, i) => (
          <div key={room.id} style={{ background: 'var(--surface)', border: `1.5px solid ${room.is_active ? 'rgba(0,200,150,0.3)' : 'var(--border)'}`, borderRadius: 12, padding: '20px', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {[{ label: 'Energy Saved', val: `${(room.energy_saved_today || 0).toFixed(3)} kWh` }, { label: 'Peso Savings', val: `₱${((room.energy_saved_today || 0) * MERALCO_RATE).toFixed(2)}` }, { label: 'Status', val: room.occupancy ? 'Occupied' : 'Vacant' }].map((s, j) => (
                <div key={j} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem' }}>
                  <span style={{ color: 'var(--text-3)' }}>{s.label}</span>
                  <strong style={{ color: 'var(--text-1)' }}>{s.val}</strong>
                </div>
              ))}
            </div>
          </div>
      ...
      {rooms.length === 0 ? (
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '48px', textAlign: 'center' }}>
          <div style={{ fontSize: '0.9rem', color: 'var(--text-3)' }}>No rooms found.</div>
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '24px' }}>
            <div style={{ fontWeight: 600, fontSize: '0.9rem', color: 'var(--text-1)', marginBottom: 4 }}>Energy Saved — All Rooms</div>
            <div style={{ fontSize: '0.75rem', color: 'var(--text-3)', marginBottom: 20 }}>kWh saved per day this week</div>
            <Bar data={{ labels: weekDays, datasets: energyDatasets }} options={chartOpts} />
          </div>
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '24px' }}>
            <div style={{ fontWeight: 600, fontSize: '0.9rem', color: 'var(--text-1)', marginBottom: 4 }}>Light Hours — All Rooms</div>
            <div style={{ fontSize: '0.75rem', color: 'var(--text-3)', marginBottom: 20 }}>Hours lights were on per day</div>
            <Line data={{ labels: weekDays, datasets: hoursDatasets }} options={chartOpts} />
          </div>
        </div>
      )}
    </motion.div>
...
function TimelinePage({ rooms, selectedRoom }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
      <div style={{ marginBottom: 24 }}>
        <h2 style={{ fontFamily: "'Instrument Serif', serif", fontSize: '1.5rem', color: 'var(--text-1)', fontWeight: 400 }}>Timeline Playback — Room {selectedRoom}</h2>
        <p style={{ fontSize: '0.85rem', color: 'var(--text-3)', marginTop: 4 }}>Replay the day's events and watch the room respond in real time</p>
      </div>
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '24px', marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.75rem', color: 'var(--text-3)', marginBottom: 20 }}>
          <span>{events[0] ? new Date(events[0].timestamp).toLocaleTimeString() : '—'}</span>
          <span style={{ color: 'var(--green)', fontWeight: 500 }}>{Math.round(progress)}% through the day</span>
          <span>{events[events.length - 1] ? new Date(events[events.length - 1].timestamp).toLocaleTimeString() : '—'}</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          <button onClick={() => setPlaying(!playing)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 22px', background: playing ? 'var(--bg)' : 'var(--green)', color: playing ? 'var(--text-1)' : 'white', border: '1.5px solid var(--border)', borderRadius: 9, fontFamily: 'Geist, sans-serif', fontSize: '0.88rem', fontWeight: 500, cursor: 'pointer' }}>
            {playing ? '⏸ Pause' : '▶ Play'}
          </button>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: '0.78rem', color: 'var(--text-3)' }}>Speed:</span>
            {[0.5, 1, 2, 4].map(s => (
              <button key={s} onClick={() => setSpeed(s)} style={{ padding: '5px 10px', borderRadius: 6, fontSize: '0.75rem', fontWeight: 600, border: `1px solid ${speed === s ? 'var(--green)' : 'var(--border)'}`, background: speed === s ? 'var(--green-dim)' : 'var(--bg)', color: speed === s ? 'var(--green)' : 'var(--text-3)', cursor: 'pointer' }}>{s}×</button>
            ))}
          </div>
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8, padding: '7px 14px', background: lightsOnAtCurrent ? 'rgba(0,200,150,0.1)' : 'rgba(239,68,68,0.08)', border: `1px solid ${lightsOnAtCurrent ? 'rgba(0,200,150,0.3)' : 'rgba(239,68,68,0.2)'}`, borderRadius: 8 }}>
            <span style={{ width: 8, height: 8, borderRadius: '50%', background: lightsOnAtCurrent ? 'var(--green)' : 'var(--red)', display: 'inline-block', animation: lightsOnAtCurrent ? 'livePulse 1.8s infinite' : 'none' }} />
            <span style={{ fontSize: '0.8rem', fontWeight: 600, color: lightsOnAtCurrent ? 'var(--green)' : 'var(--red)' }}>Room {selectedRoom} — Lights {lightsOnAtCurrent ? 'ON' : 'OFF'}</span>
          </div>
        </div>
      </div>
      {currentEvent && (
        <motion.div key={currentEvent.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} style={{ background: currentEvent.status === 'on' ? 'rgba(0,200,150,0.08)' : 'rgba(239,68,68,0.06)', border: `1px solid ${currentEvent.status === 'on' ? 'rgba(0,200,150,0.25)' : 'rgba(239,68,68,0.2)'}`, borderRadius: 10, padding: '14px 18px', marginBottom: 20, display: 'flex', gap: 16, alignItems: 'center' }}>
          <div style={{ width: 36, height: 36, borderRadius: '50%', background: currentEvent.status === 'on' ? 'var(--green-dim)' : 'rgba(239,68,68,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.1rem' }}>{currentEvent.status === 'on' ? '💡' : '🌑'}</div>
          <div>
            <div style={{ fontWeight: 600, fontSize: '0.9rem', color: 'var(--text-1)' }}>{currentEvent.event_type}</div>
            <div style={{ fontSize: '0.78rem', color: 'var(--text-3)', marginTop: 2 }}>{new Date(currentEvent.timestamp).toLocaleString()} · Duration: {currentEvent.duration || '—'}</div>
          </div>
        </motion.div>
      )}
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '20px 24px' }}>
        ...
        {events.length === 0 ? (
          <div style={{ textAlign: 'center', color: 'var(--text-3)', fontSize: '0.85rem', padding: '24px' }}>No events to replay. Simulate motion first.</div>
        ) : (
          <div style={{ position: 'relative' }}>
            <div style={{ position: 'absolute', left: 16, top: 0, bottom: 0, width: 2, background: 'var(--border)' }} />
            <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
              {displayedEvents.slice(-12).map((ev, i) => {
                const isActive = ev === activeEvent
                return (
                  <motion.div key={i} initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '8px 0', paddingLeft: 8 }}>
                    <div style={{ width: 10, height: 10, borderRadius: '50%', border: `2px solid ${ev.status === 'on' ? 'var(--green)' : 'var(--red)'}`, background: isActive ? (ev.status === 'on' ? 'var(--green)' : 'var(--red)') : 'white', flexShrink: 0, zIndex: 1 }} />
                    <div style={{ fontSize: '0.78rem', color: 'var(--text-3)', width: 70, flexShrink: 0 }}>{new Date(ev.timestamp).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}</div>
                    <div style={{ fontSize: '0.82rem', color: isActive ? 'var(--text-1)' : 'var(--text-2)', fontWeight: isActive ? 600 : 400 }}>{ev.event_type}</div>
                    <span style={{ marginLeft: 'auto', fontSize: '0.7rem', fontWeight: 600, padding: '2px 8px', borderRadius: 99, background: ev.status === 'on' ? '#d1fae5' : '#fee2e2', color: ev.status === 'on' ? '#065f46' : '#991b1b' }}>{ev.status === 'on' ? 'Active' : 'Ended'}</span>
                  </motion.div>
                )
              })}
            </div>
    ...
    <div className="esp32-card">
      <div className="esp32-header">
        ...
        <span className={`esp32-badge ${isConnected ? 'esp32-connected' : 'esp32-offline'}`}>
          <span className="esp32-dot" />{isConnected ? 'Connected' : 'Offline'}
        </span>
      </div>
...
function ReportPage({ rooms, selectedRoom }) {
  ...
  const energySaved = room?.energy_saved_today || 0
  const pesoCost = (energySaved * MERALCO_RATE).toFixed(2)
    ...
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
      <div className="report-header">
        <div>
          ...
          <p className="report-sub">Summary of energy usage, occupancy, and savings</p>
        </div>
        <button className="export-btn" onClick={handleExport}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>Export CSV</button>
      </div>
      <div className="report-stat-grid">
        {[
          ...
          <div key={i} className="report-stat-card">
            <div className="report-stat-icon" style={{ background: s.bg }}>{s.icon}</div>
      ...
      <div className="charts-row" style={{ marginBottom: 24 }}>
        <div className="chart-panel">
          <div className="chart-panel-head"><span className="chart-panel-title">Event Breakdown</span><span className="chart-panel-sub">ON vs OFF events ratio</span></div>
          <div style={{ maxWidth: 220, margin: '0 auto' }}>
            <Doughnut data={{ labels: ['Lights ON', 'Lights OFF'], datasets: [{ data: [onEvents || 1, offEvents || 1], backgroundColor: ['rgba(0,200,150,0.8)', 'rgba(239,68,68,0.7)'], borderWidth: 0 }] }} options={{ plugins: { legend: { position: 'bottom', labels: { color: '#4a5e52', font: { size: 12 } } }, tooltip: { backgroundColor: '#0e1a14', titleColor: '#fff', bodyColor: '#8fa898' } }, cutout: '65%' }} />
          </div>
        </div>
        <div className="chart-panel">
          <div className="chart-panel-head"><span className="chart-panel-title">Energy Cost Calculator</span><span className="chart-panel-sub">Based on Meralco rate ₱{MERALCO_RATE}/kWh</span></div>
          <div className="cost-breakdown">
            <div className="cost-row"><span>Energy saved today</span><strong>{energySaved.toFixed(3)} kWh</strong></div>
            <div className="cost-row"><span>Total hours ON</span><strong>{hoursOn} hrs</strong></div>
            <div className="cost-row"><span>Meralco rate</span><strong>₱{MERALCO_RATE}/kWh</strong></div>
            <div className="cost-row cost-total"><span>Estimated Savings</span><strong style={{ color: 'var(--green)', fontSize: '1.2rem' }}>₱{pesoCost}</strong></div>
            <div className="cost-note">Savings compared to leaving lights on all day (8hrs baseline).</div>
          </div>
        </div>
      </div>
    </motion.div>
  ...
  const tutorialSteps = [
    {
      ...
      desc: 'You need: ESP32 board, PIR motion sensor (HC-SR501), 5V Relay module, 2.5V bulb with socket, 9V battery with connector, breadboard, and jumper wires.',
      detail: 'Make sure your relay module is rated for at least 5V coil voltage. The 9V battery connects to the relay output side to power the 2.5V bulb through a resistor.',
    },
    {
      num: 2,
      color: '#6366f1',
      title: 'Wire the PIR Sensor to ESP32',
      desc: 'PIR VCC → 3.3V pin on ESP32. PIR GND → GND on ESP32. PIR OUT → GPIO 13 on ESP32.',
      detail: 'The PIR sensor outputs HIGH (3.3V) when motion is detected and LOW when idle. GPIO 13 reads this signal. No resistor needed for the PIR data line.',
    },
    {
      num: 3,
      color: '#f59e0b',
      title: 'Wire the Relay Module to ESP32',
      desc: 'Relay VCC → 5V pin (VIN) on ESP32. Relay GND → GND on ESP32. Relay IN → GPIO 2 on ESP32.',
      detail: 'GPIO 2 controls the relay coil. When ESP32 sends HIGH to GPIO 2, the relay closes its switch. Use the NO (Normally Open) terminal on the relay for the bulb circuit.',
    },
    {
      num: 4,
      color: '#ef4444',
      title: 'Wire the 9V Battery + Resistor + Bulb',
      desc: 'Battery (+) → Relay COM terminal. Relay NO terminal → Resistor (680Ω) → Bulb (+). Bulb (−) → Battery (−).',
      detail: 'The 9V battery powers the bulb through the relay. Since your bulb is 2.5V 3W, use a 680Ω resistor (9V − 2.5V = 6.5V drop, 6.5 ÷ 0.012A ≈ 540Ω, round up to 680Ω for safety). This protects your tiny bulb from burning out.',
    },
    {
      num: 5,
      color: '#8b5cf6',
      title: 'Connect ESP32 to Phone Hotspot',
      desc: 'Enable hotspot on your phone: name it "ohyoudonthaveloadhowpoor", password "gelatninjer". The ESP32 code already has these credentials.',
      detail: 'Make sure your Django backend server is also on the same network or has a public IP. Replace YOUR_SERVER_IP in the code with your laptop\'s IP address (check with ipconfig on Windows or ifconfig on Mac/Linux).',
    },
    {
      num: 6,
      color: '#10b981',
      title: 'Flash the Code via Arduino IDE',
      desc: 'Install Arduino IDE → Add ESP32 board URL in preferences → Install "ESP32 by Espressif" from board manager → Select your board → Upload.',
      detail: 'Board manager URL: https://raw.githubusercontent.com/espressif/arduino-esp32/gh-pages/package_esp32_index.json — Select board: "ESP32 Dev Module". Open Serial Monitor at 115200 baud to see debug logs.',
    },
    {
      num: 7,
      color: '#f59e0b',
      title: 'Test the Full System',
      desc: 'Power on ESP32 → watch Serial Monitor for WiFi connection → wave your hand in front of PIR → bulb should turn ON and dashboard should update.',
      detail: 'If nothing happens: check your server IP in the code, make sure Django is running (python manage.py runserver 0.0.0.0:8000), and verify the PIR is warmed up (takes 30-60 seconds after power-on).',
    },
  ]
      ...
      <div className="device-page">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          <div className="device-guide">
            ...
            <p className="device-guide-sub">
              Flash this to your ESP32. WiFi is pre-configured for your phone hotspot.
              Replace <code style={{ background: 'var(--bg)', padding: '2px 6px', borderRadius: 4, fontSize: '0.8rem' }}>YOUR_SERVER_IP</code> with your laptop's local IP before uploading.
            </p>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12, padding: '10px 14px', background: 'rgba(0,200,150,0.06)', border: '1px solid rgba(0,200,150,0.2)', borderRadius: 8 }}>
              ...
              <span style={{ fontSize: '0.8rem', color: 'var(--text-2)' }}>
                Hotspot: <strong style={{ color: 'var(--text-1)' }}>ohyoudonthaveloadhowpoor</strong> · Password: <strong style={{ color: 'var(--text-1)' }}>gelatninjer</strong>
              </span>
            </div>
            ...
            <div className="device-pins">
              {[['PIR VCC', '3.3V or 5V'], ['PIR GND', 'GND'], ['PIR OUT', 'GPIO 13'], ['Relay IN', 'GPIO 2'], ['Relay VCC', '5V (VIN)'], ['Relay GND', 'GND'], ['Bulb (+)', 'Relay NO'], ['Bulb (−)', 'Battery (−)'], ['Battery (+)', 'Relay COM'], ['Resistor', '680Ω in series']].map(([l, v], i) => (
                <div key={i} className="pin-row"><span className="pin-label">{l}</span><span className="pin-val">{v}</span></div>
              ))}
            </div>
          </div>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '28px' }}>
            ...
            <p style={{ fontSize: '0.85rem', color: 'var(--text-3)', marginBottom: 24, lineHeight: 1.6 }}>
              Follow each step carefully. Click any step to expand details. Your bulb is 2.5V 3W — the resistor is critical to prevent burning it out.
            </p>

            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 14, marginBottom: 28 }}>
              {[
                { icon: '🔲', name: 'ESP32', desc: 'Main microcontroller', color: '#1a5c22' },
                { icon: '👁️', name: 'PIR Sensor', desc: 'HC-SR501 motion detector', color: '#cc4444' },
                { icon: '⚡', name: 'Relay Module', desc: '5V coil, switches bulb', color: '#0a3a7a' },
                { icon: '💡', name: '2.5V Bulb', desc: '3W, tiny socket bulb', color: '#c87820' },
                { icon: '🔋', name: '9V Battery', desc: 'Powers bulb circuit', color: '#cc2200' },
                { icon: '🟫', name: 'Breadboard', desc: 'Prototyping connections', color: '#888' },
                { icon: '〰️', name: '680Ω Resistor', desc: 'Protects 2.5V bulb', color: '#c8a864' },
                { icon: '🔌', name: 'Jumper Wires', desc: 'Male-to-male + M-to-F', color: '#4488ff' },
              ].map((comp, i) => (
                <div key={i} style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 10, padding: '16px', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: 8, transition: 'box-shadow 0.2s' }}>
                  <div style={{ width: 48, height: 48, borderRadius: 12, background: `${comp.color}18`, border: `1.5px solid ${comp.color}44`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.5rem' }}>{comp.icon}</div>
                  <div style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-1)' }}>{comp.name}</div>
                  <div style={{ fontSize: '0.72rem', color: 'var(--text-3)', lineHeight: 1.5 }}>{comp.desc}</div>
                </div>
              ))}
            </div>
                      ...
                      <AnimatePresence>
                        {activeStep === step.num && (
                          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} style={{ overflow: 'hidden' }}>
                            <div style={{ marginTop: 12, padding: '12px 14px', background: `${step.color}10`, border: `1px solid ${step.color}30`, borderRadius: 8, fontSize: '0.8rem', color: 'var(--text-2)', lineHeight: 1.7 }}>
                              💡 {step.detail}
                            </div>
                          </motion.div>
                        )}
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '28px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8, flexWrap: 'wrap', gap: 12 }}>
              <div>
                <h3 style={{ fontFamily: "'Instrument Serif', serif", fontSize: '1.3rem', color: 'var(--text-1)', marginBottom: 4 }}>3D Circuit Wiring Model</h3>
                <p style={{ fontSize: '0.82rem', color: 'var(--text-3)', lineHeight: 1.6 }}>
                  Interactive 3D view of your exact components. Drag to orbit · Scroll to zoom · Right-click to pan.
                  Colored wires show data flow direction.
                </p>
              </div>
              <button
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }}>
              {[
                { color: '#ff4444', label: 'VCC / Power' },
                { color: '#222222', label: 'GND' },
                { color: '#00c896', label: 'PIR Signal (GPIO 13)' },
                { color: '#4488ff', label: 'Relay Control (GPIO 2)' },
                { color: '#ff6600', label: 'Bulb Circuit (9V)' },
                { color: '#ff8844', label: 'Relay Output' },
              ].map((item, i) => (
            ...
            <div style={{ marginTop: 14, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
              {[
                { title: 'PIR → ESP32', desc: 'GPIO 13 reads HIGH when motion detected', icon: '👁️' },
                { title: 'ESP32 → Relay', desc: 'GPIO 2 sends signal to switch relay coil', icon: '⚡' },
                { title: 'Relay → Bulb', desc: '9V battery powers 2.5V bulb via relay + resistor', icon: '💡' },
              ].map((info, i) => (
...
function DashboardScreen({ onLogout }) {
    ...
    try {
      ...
      setRooms(prev => {
        ...
        if (prevRoom && newRoom && newRoom.events?.length > prevEventCount.current) {
          ...
          if (silent) addToast(`Motion detected in Room ${selectedRoom}!`, 'success')
        }
  ...
  useEffect(() => { loadRooms() }, [])
  useEffect(() => {
    pollRef.current = setInterval(() => { if (!viewingDate) loadRooms(true) }, POLL_INTERVAL)
  ...
  const handleSimulate = async () => {
    ...
    setSimulating(true)
    try { await simulateMotion(selectedRoom); await loadRooms(); setViewingDate(null); addToast('Lights ON — motion simulated!', 'success') }
    catch { addToast('Simulation failed', 'warning') }
    finally { setSimulating(false) }
  }
  ...
  let isEmpty = true, isFuture = false
  ...
  const pageTitles = { dashboard: 'Dashboard', rooms: 'Rooms', logs: 'Event Logs', report: 'Reports', device: 'ESP32 Device', classroom3d: '3D Classroom', insights: 'Smart Insights', automation: 'Automation', simulation: 'Simulation', timeline: 'Timeline', multiroom: 'Multi-Room' }

  const navGroups = [
    { label: 'Monitor', keys: ['dashboard', 'rooms', 'logs'] },
    { label: 'Analyze', keys: ['report', 'multiroom', 'insights', 'timeline'] },
    { label: 'Control', keys: ['automation', 'simulation'] },
    { label: 'System', keys: ['device', 'classroom3d'] },
  ]
              ...
              <div className="rooms-grid">
                {rooms.map(room => (
                  <div key={room.id} className={`room-card ${room.is_active ? 'room-active' : ''}`} onClick={() => { setSelectedRoom(room.name); setActiveNav('dashboard') }}>
                    <div className="room-card-top"><span className="room-card-name">Room {room.name}</span><StatusBadge on={room.is_active} /></div>
                    <div className="room-card-stat"><span>{room.occupancy ? 'Occupied' : 'Vacant'}</span><span>{room.energy_saved_today.toFixed(2)} kWh saved</span></div>
                    <div className="room-card-motion">Last motion: {room.last_motion ? new Date(room.last_motion).toLocaleTimeString() : 'Never'}</div>
                  </div>
                ))}
                {rooms.length === 0 && <p style={{ color: 'var(--text-3)' }}>No rooms found.</p>}
              </div>
            </motion.div>
          )}
            ...
            <>
              {historyLoading ? (
                ...
                <motion.div className="empty-state" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                  ...
                  <p className="empty-body">No activity recorded. Simulate motion or connect your ESP32 to begin.</p>
                  {!viewingDate && (
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                  ...
                  {!viewingDate && (
                    <div className="sim-btns" style={{ marginTop: 20 }}>
                      <button className="simulate-btn" onClick={handleSimulate} disabled={simulating}>{simulating ? 'Simulating…' : '⚡ Simulate Lights ON'}</button>
