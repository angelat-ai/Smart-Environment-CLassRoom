    ...
    setTimeout(() => {
      try {
        ...
        if (energySaved < 0.3 && hoursOn > 15) {
          generatedInsights.push({ type: 'warning', title: 'Critical Energy Waste Detected', body: `Room ${selectedRoom} consumed ${hoursOn.toFixed(1)} hours this week but saved only ${energySaved.toFixed(3)} kWh today. Lights may be staying on during vacant periods or the auto-off timeout is too long.`, saving: `~₱${(hoursOn * 0.12 * ZAMCELCO_RATE).toFixed(2)}/week potential savings`, action: 'Reduce timeout to 5 minutes' })
        } else if (energySaved > 1.0) {
          generatedInsights.push({ type: 'success', title: 'Excellent Energy Performance', body: `Room ${selectedRoom} saved ${energySaved.toFixed(3)} kWh today (${(energySaved * ZAMCELCO_RATE).toFixed(2)} pesos). The automation prevented ${totalEvents - onEvents} unnecessary light activations.`, saving: `₱${(energySaved * ZAMCELCO_RATE).toFixed(2)} saved today`, action: 'Maintain current settings' })
        } else {
          generatedInsights.push({ type: 'info', title: 'Moderate Energy Usage', body: `Room ${selectedRoom} shows ${energySaved.toFixed(3)} kWh saved today. With ${hoursOn.toFixed(1)} total hours this week, efficiency is average.`, saving: `₱${(energySaved * ZAMCELCO_RATE).toFixed(2)} saved`, action: 'Monitor for 3 more days' })
        }
        
        if (occupancyRate < 20 && totalEvents > 3) {
          generatedInsights.push({ type: 'warning', title: 'Sensor Sensitivity Issue', body: `Only ${occupancyRate}% occupancy rate with ${totalEvents} events suggests the PIR sensor may be positioned incorrectly or sensitivity is too low.`, saving: '~20% more accurate detection possible', action: 'Reposition sensor toward seating area' })
        } else if (occupancyRate > 75) {
          generatedInsights.push({ type: 'success', title: 'High Classroom Utilization', body: `${occupancyRate}% occupancy rate on ${today} shows Room ${selectedRoom} is actively used.`, saving: 'Optimal classroom usage pattern', action: 'No action needed' })
        } else {
          generatedInsights.push({ type: 'tip', title: 'Balanced Occupancy Pattern', body: `At ${occupancyRate}% occupancy, Room ${selectedRoom} has a healthy mix of active use and idle time.`, saving: 'Efficiency maintained', action: 'Continue monitoring' })
        }
        
        if (maxHours > 8) {
          generatedInsights.push({ type: 'warning', title: 'Extended Light Usage Alert', body: `Peak usage of ${maxHours} hours on ${peakDay?.date || 'one day'} exceeds standard classroom hours.`, saving: `~₱${((maxHours - 6) * ZAMCELCO_RATE * 0.09).toFixed(2)}/day if corrected`, action: 'Check relay connections' })
        } else if (avgHours < 2) {
          generatedInsights.push({ type: 'info', title: 'Minimal Light Usage', body: `Averaging only ${avgHours} hours daily suggests Room ${selectedRoom} is rarely used.`, saving: 'Low consumption baseline', action: 'Verify room schedule' })
        } else {
          generatedInsights.push({ type: 'success', title: 'Healthy Daily Light Average', body: `At ${avgHours} hours per day, Room ${selectedRoom} aligns with typical class schedules.`, saving: 'Within expected parameters', action: 'No changes needed' })
        }
        
        const lastEvent = events[0]
        if (lastEvent && lastEvent.status === 'on' && room?.occupancy === false) {
          generatedInsights.push({ type: 'warning', title: 'Lights On While Vacant', body: `Latest event at ${new Date(lastEvent.timestamp).toLocaleTimeString()} shows lights ON but room is vacant.`, saving: `~₱${(0.5 * ZAMCELCO_RATE).toFixed(2)}/day`, action: 'Verify GPIO 2 output' })
        } else if (lastEvent && lastEvent.status === 'off') {
          generatedInsights.push({ type: 'success', title: 'Auto-Off Working Correctly', body: `Last event at ${new Date(lastEvent.timestamp).toLocaleTimeString()} shows lights turned OFF successfully.`, saving: 'System operational', action: 'No action needed' })
        }
        
        if (totalSaved > 2.0) {
          generatedInsights.push({ type: 'success', title: 'Weekly Savings Milestone', body: `Total savings of ${totalSaved.toFixed(3)} kWh this week equals ₱${(totalSaved * ZAMCELCO_RATE).toFixed(2)}.`, saving: `₱${(totalSaved * ZAMCELCO_RATE).toFixed(2)} this week`, action: 'Track monthly trend' })
        }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 24 }}>
        {[
          { label: 'Energy Today', val: `${(room?.energy_saved_today || 0).toFixed(3)} kWh`, color: '#00c896' },
          { label: 'Peso Savings', val: `₱${((room?.energy_saved_today || 0) * ZAMCELCO_RATE).toFixed(2)}`, color: '#f59e0b' },
          { label: 'Light Status', val: room?.is_active ? 'ON' : 'OFF', color: room?.is_active ? '#00c896' : '#ef4444' },
          { label: 'Occupancy', val: room?.occupancy ? 'Occupied' : 'Vacant', color: room?.occupancy ? '#6366f1' : '#8fa898' },
        ].map((s, i) => (
          <div key={i} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, padding: '16px', textAlign: 'center' }}>
            <div style={{ fontSize: '0.72rem', color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>{s.label}</div>
            <div style={{ fontFamily: "'Instrument Serif', serif", fontSize: '1.5rem', color: s.color }}>{s.val}</div>
          </div>
        ))}
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {savedInsights.length === 0 ? (
            <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '48px 32px', textAlign: 'center' }}>
              ...
              <div style={{ fontFamily: "'Instrument Serif', serif", fontSize: '1.2rem', color: 'var(--text-1)', marginBottom: 6 }}>No Saved Insights</div>
              <div style={{ fontSize: '0.85rem', color: 'var(--text-3)', lineHeight: 1.6 }}>Generate insights and click Save to store them here for future reference.</div>
            </div>
          ) : (
            savedInsights.map((insight, i) => {
              ...
              return (
                <motion.div key={insight.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} style={{ background: cfg.bg, border: `1px solid ${cfg.border}`, borderRadius: 12, padding: '18px 20px' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6 }}>
                        <div style={{ fontSize: '0.92rem', fontWeight: 600, color: 'var(--text-1)' }}>{insight.title}</div>
                        <span style={{ fontSize: '0.68rem', color: 'var(--text-3)', whiteSpace: 'nowrap' }}>{new Date(insight.date).toLocaleDateString()}</span>
                      </div>
                  ...
                  <div style={{ marginTop: 12, paddingTop: 10, borderTop: `1px solid ${cfg.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '0.72rem', color: 'var(--text-3)' }}>Room {insight.room}</span>
                    <button onClick={() => onDeleteInsight(insight.id)} style={{ padding: '4px 10px', background: 'none', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 6, fontSize: '0.72rem', color: 'var(--red)', fontWeight: 500, cursor: 'pointer', fontFamily: 'Geist, sans-serif' }}>Delete</button>
                  </div>
                </motion.div>
              )
            })
...
function AutomationPage({ addToast }) {
  const [rules, setRules] = useState(() => {
    try { return JSON.parse(localStorage.getItem('sec_rules') || '[]') } catch { return [] }
  })
  ...
  const addRule = () => {
    ...
    const rule = { ...form, id: Date.now(), createdAt: new Date().toISOString() }
    saveRules([...rules, rule])
    setForm({ name: '', trigger: 'no_motion', delay: 5, action: 'lights_off', room: 'A', enabled: true })
    setAdding(false)
    ...
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ fontSize: '0.85rem', color: 'var(--text-3)', marginTop: 4 }}>Define smart triggers and actions for automatic light control</p>
        </div>
        <button onClick={() => setAdding(!adding)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 18px', background: adding ? 'var(--bg)' : 'var(--dark)', color: adding ? 'var(--text-2)' : 'var(--green)', border: `1.5px solid ${adding ? 'var(--border)' : 'rgba(0,200,150,0.3)'}`, borderRadius: 9, fontFamily: 'Geist, sans-serif', fontSize: '0.83rem', fontWeight: 500, cursor: 'pointer' }}>{adding ? '✕ Cancel' : '+ Add Rule'}</button>
      </div>
      <AnimatePresence>
        {adding && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} style={{ background: 'var(--surface)', border: '1px solid var(--green)', borderRadius: 12, padding: '24px', marginBottom: 20, overflow: 'hidden' }}>
            <div style={{ fontWeight: 600, fontSize: '0.9rem', color: 'var(--text-1)', marginBottom: 16 }}>New Automation Rule</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              <div><label style={{ fontSize: '0.75rem', color: 'var(--text-3)', fontWeight: 500, display: 'block', marginBottom: 6 }}>Rule Name</label><input value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} placeholder="e.g. Auto lights off after class" style={{ width: '100%', padding: '9px 12px', border: '1.5px solid var(--border)', borderRadius: 8, fontFamily: 'Geist, sans-serif', fontSize: '0.85rem', color: 'var(--text-1)', background: 'var(--bg)', outline: 'none', boxSizing: 'border-box' }} /></div>
              <div><label style={{ fontSize: '0.75rem', color: 'var(--text-3)', fontWeight: 500, display: 'block', marginBottom: 6 }}>Room</label><select value={form.room} onChange={e => setForm(p => ({ ...p, room: e.target.value }))} style={{ width: '100%', padding: '9px 12px', border: '1.5px solid var(--border)', borderRadius: 8, fontFamily: 'Geist, sans-serif', fontSize: '0.85rem', color: 'var(--text-1)', background: 'var(--bg)', outline: 'none' }}>{['A','B','C'].map(r => <option key={r}>Room {r}</option>)}</select></div>
              <div><label style={{ fontSize: '0.75rem', color: 'var(--text-3)', fontWeight: 500, display: 'block', marginBottom: 6 }}>Trigger (IF)</label><select value={form.trigger} onChange={e => setForm(p => ({ ...p, trigger: e.target.value }))} style={{ width: '100%', padding: '9px 12px', border: '1.5px solid var(--border)', borderRadius: 8, fontFamily: 'Geist, sans-serif', fontSize: '0.85rem', color: 'var(--text-1)', background: 'var(--bg)', outline: 'none' }}>{Object.entries(triggerLabels).map(([v, l]) => <option key={v} value={v}>{l}</option>)}</select></div>
              <div><label style={{ fontSize: '0.75rem', color: 'var(--text-3)', fontWeight: 500, display: 'block', marginBottom: 6 }}>Action (THEN)</label><select value={form.action} onChange={e => setForm(p => ({ ...p, action: e.target.value }))} style={{ width: '100%', padding: '9px 12px', border: '1.5px solid var(--border)', borderRadius: 8, fontFamily: 'Geist, sans-serif', fontSize: '0.85rem', color: 'var(--text-1)', background: 'var(--bg)', outline: 'none' }}>{Object.entries(actionLabels).map(([v, l]) => <option key={v} value={v}>{l}</option>)}</select></div>
              <div><label style={{ fontSize: '0.75rem', color: 'var(--text-3)', fontWeight: 500, display: 'block', marginBottom: 6 }}>Delay (minutes)</label><input type="number" min={1} max={60} value={form.delay} onChange={e => setForm(p => ({ ...p, delay: parseInt(e.target.value) || 5 }))} style={{ width: '100%', padding: '9px 12px', border: '1.5px solid var(--border)', borderRadius: 8, fontFamily: 'Geist, sans-serif', fontSize: '0.85rem', color: 'var(--text-1)', background: 'var(--bg)', outline: 'none', boxSizing: 'border-box' }} /></div>
              <div style={{ display: 'flex', alignItems: 'flex-end' }}><button onClick={addRule} style={{ width: '100%', padding: '10px', background: 'var(--green)', color: 'white', border: 'none', borderRadius: 9, fontFamily: 'Geist, sans-serif', fontSize: '0.88rem', fontWeight: 500, cursor: 'pointer' }}>Save Rule</button></div>
            </div>
            <div style={{ marginTop: 14, padding: '10px 14px', background: 'rgba(0,200,150,0.06)', borderRadius: 8, fontSize: '0.82rem', color: 'var(--text-2)' }}><strong style={{ color: 'var(--text-1)' }}>Preview: </strong>IF <strong>{triggerLabels[form.trigger]}</strong> for <strong>{form.delay} min</strong> in <strong>Room {form.room}</strong> → THEN <strong>{actionLabels[form.action]}</strong></div>
          </motion.div>
        )}
      </AnimatePresence>
      {rules.length === 0 ? (
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '48px 32px', textAlign: 'center' }}><div style={{ fontSize: '2.5rem', marginBottom: 16 }}>⚙️</div><div style={{ fontFamily: "'Instrument Serif', serif", fontSize: '1.3rem', color: 'var(--text-1)', marginBottom: 8 }}>No Rules Yet</div><div style={{ fontSize: '0.85rem', color: 'var(--text-3)', lineHeight: 1.7 }}>Create automation rules to automatically control lights based on motion, time, or occupancy.</div></div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {rules.map((rule, i) => (
            <motion.div key={rule.id} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }} style={{ background: 'var(--surface)', border: `1.5px solid ${rule.enabled ? 'rgba(0,200,150,0.2)' : 'var(--border)'}`, borderRadius: 12, padding: '18px 20px', display: 'flex', alignItems: 'center', gap: 16, opacity: rule.enabled ? 1 : 0.6 }}>
              <div onClick={() => toggleRule(rule.id)} style={{ width: 38, height: 22, borderRadius: 11, cursor: 'pointer', flexShrink: 0, background: rule.enabled ? 'var(--green)' : 'var(--border)', position: 'relative', transition: 'background 0.2s' }}><div style={{ width: 16, height: 16, borderRadius: '50%', background: 'white', position: 'absolute', top: 3, left: rule.enabled ? 19 : 3, transition: 'left 0.2s', boxShadow: '0 1px 4px rgba(0,0,0,0.2)' }} /></div>
              <div style={{ flex: 1 }}><div style={{ fontWeight: 600, fontSize: '0.9rem', color: 'var(--text-1)', marginBottom: 4 }}>{rule.name}</div><div style={{ fontSize: '0.8rem', color: 'var(--text-3)' }}>IF <span style={{ color: 'var(--text-2)', fontWeight: 500 }}>{triggerLabels[rule.trigger]}</span> for {rule.delay}m in Room {rule.room}{' → '}<span style={{ color: 'var(--green)', fontWeight: 500 }}>{actionLabels[rule.action]}</span></div></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><span style={{ fontSize: '0.7rem', fontWeight: 600, padding: '3px 9px', borderRadius: 99, background: rule.enabled ? 'rgba(0,200,150,0.1)' : 'rgba(0,0,0,0.05)', color: rule.enabled ? 'var(--green)' : 'var(--text-3)' }}>{rule.enabled ? 'Active' : 'Paused'}</span><button onClick={() => deleteRule(rule.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-3)', padding: '4px', borderRadius: 6 }}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4h6v2"/></svg></button></div>
            </motion.div>
          ))}
        </div>
      )}
...
function SimulationPage({ rooms, selectedRoom, loadRooms, addToast }) {
  ...
  const scenarios = [
    { id: 'morning', name: 'Morning Class', desc: 'Students arrive at 7AM, class runs 3 hours, then room clears.', steps: [{ label: 'Students arriving…', action: 'on', delay: 600 }, { label: 'Class in session', action: null, delay: 2000 }, { label: 'Break time', action: 'off', delay: 1000 }, { label: 'Class resumes', action: 'on', delay: 1500 }, { label: 'Class ends', action: 'off', delay: 800 }] },
    { id: 'idle', name: 'Idle Detection', desc: 'Lights left on with no occupancy — tests auto-off behavior.', steps: [{ label: 'Lights turned ON', action: 'on', delay: 600 }, { label: 'No motion detected', action: null, delay: 2000 }, { label: 'Idle timeout triggered', action: null, delay: 1500 }, { label: 'Auto lights OFF', action: 'off', delay: 800 }] },
  ...
  const runScenario = async (sc) => {
    setScenario(sc.id); setScenarioRunning(true); setScenarioLog([]); setScenarioProgress(0)
    ...
    for (let i = 0; i < sc.steps.length; i++) {
      ...
      setScenarioProgress(Math.round(((i + 1) / total) * 100))
    ...
    addToast(`Scenario "${sc.name}" complete!`, 'success')
  }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '24px', marginBottom: 20 }}>
        <div style={{ fontWeight: 600, fontSize: '0.95rem', color: 'var(--text-1)', marginBottom: 4 }}>Manual Controls — Room {selectedRoom}</div>
        <div style={{ fontSize: '0.8rem', color: 'var(--text-3)', marginBottom: 20 }}>Instantly trigger motion ON or lights OFF for the selected room.</div>
        <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
          <button onClick={handleOn} disabled={simulating || scenarioRunning} style={{ padding: '11px 28px', background: 'none', border: '1.5px solid var(--green)', borderRadius: 9, color: 'var(--green)', fontFamily: 'Geist, sans-serif', fontSize: '0.88rem', fontWeight: 500, cursor: 'pointer', transition: 'all 0.18s' }} onMouseEnter={e => { e.target.style.background = 'var(--green)'; e.target.style.color = 'white' }} onMouseLeave={e => { e.target.style.background = 'none'; e.target.style.color = 'var(--green)' }}>{simulating ? 'Simulating…' : '⚡ Lights ON'}</button>
          <button onClick={handleOff} disabled={simulatingOff || scenarioRunning} style={{ padding: '11px 28px', background: 'none', border: '1.5px solid var(--red)', borderRadius: 9, color: 'var(--red)', fontFamily: 'Geist, sans-serif', fontSize: '0.88rem', fontWeight: 500, cursor: 'pointer', transition: 'all 0.18s' }} onMouseEnter={e => { e.target.style.background = 'var(--red)'; e.target.style.color = 'white' }} onMouseLeave={e => { e.target.style.background = 'none'; e.target.style.color = 'var(--red)' }}>{simulatingOff ? 'Turning off…' : '🌑 Lights OFF'}</button>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginLeft: 8 }}><div style={{ width: 10, height: 10, borderRadius: '50%', background: room?.is_active ? 'var(--green)' : '#ccc', animation: room?.is_active ? 'livePulse 1.8s infinite' : 'none' }} /><span style={{ fontSize: '0.82rem', color: 'var(--text-2)' }}>Current: <strong>{room?.is_active ? 'ON' : 'OFF'}</strong></span></div>
        </div>
      </div>
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '24px', marginBottom: scenarioRunning ? 20 : 0 }}>
        <div style={{ fontWeight: 600, fontSize: '0.95rem', color: 'var(--text-1)', marginBottom: 4 }}>Scenario Runner</div>
        <div style={{ fontSize: '0.8rem', color: 'var(--text-3)', marginBottom: 20 }}>Run a realistic multi-step simulation to test system behavior over time.</div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14 }}>
          {scenarios.map(sc => (
            <div key={sc.id} style={{ background: scenario === sc.id ? 'rgba(0,200,150,0.06)' : 'var(--bg)', border: `1.5px solid ${scenario === sc.id ? 'rgba(0,200,150,0.3)' : 'var(--border)'}`, borderRadius: 10, padding: '18px' }}>
              <div style={{ fontWeight: 600, fontSize: '0.88rem', color: 'var(--text-1)', marginBottom: 6 }}>{sc.name}</div>
              <div style={{ fontSize: '0.78rem', color: 'var(--text-3)', lineHeight: 1.6, marginBottom: 14 }}>{sc.desc}</div>
              <div style={{ fontSize: '0.72rem', color: 'var(--text-3)', marginBottom: 12 }}>{sc.steps.length} steps</div>
              <button onClick={() => runScenario(sc)} disabled={scenarioRunning} style={{ width: '100%', padding: '8px', background: scenario === sc.id && scenarioRunning ? 'var(--green-dim)' : 'var(--dark)', color: 'var(--green)', border: '1px solid rgba(0,200,150,0.2)', borderRadius: 7, fontFamily: 'Geist, sans-serif', fontSize: '0.8rem', fontWeight: 500, cursor: scenarioRunning ? 'not-allowed' : 'pointer', opacity: scenarioRunning && scenario !== sc.id ? 0.5 : 1 }}>{scenario === sc.id && scenarioRunning ? '▶ Running…' : '▶ Run'}</button>
            </div>
          ))}
        </div>
      </div>
      <AnimatePresence>
        {scenarioRunning && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} style={{ background: 'var(--dark)', borderRadius: 12, padding: '20px 24px', marginTop: 0, overflow: 'hidden' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}><span style={{ color: 'var(--green)', fontFamily: 'monospace', fontSize: '0.85rem', fontWeight: 600 }}>● Scenario Running</span><span style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.78rem' }}>{scenarioProgress}%</span></div>
            <div style={{ height: 4, background: 'rgba(255,255,255,0.08)', borderRadius: 99, marginBottom: 16, overflow: 'hidden' }}><motion.div style={{ height: '100%', background: 'var(--green)', borderRadius: 99 }} animate={{ width: `${scenarioProgress}%` }} /></div>
            {scenarioLog.map((step, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                <span style={{ fontSize: '0.7rem', color: step.status === 'done' ? 'var(--green)' : 'rgba(255,255,255,0.3)', fontFamily: 'monospace', width: 80 }}>{step.time}</span>
                <span style={{ fontSize: '0.82rem', color: step.status === 'done' ? 'rgba(255,255,255,0.8)' : 'rgba(255,255,255,0.5)' }}>{step.label}</span>
                <span style={{ marginLeft: 'auto', fontSize: '0.7rem', color: step.status === 'done' ? 'var(--green)' : '#f59e0b' }}>{step.status === 'done' ? '✓' : '…'}</span>
              </div>
            ))}
          </motion.div>
        )}
...
function MultiRoomPage({ rooms }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.min(rooms.length, 4)}, 1fr)`, gap: 14, marginBottom: 24 }}>
        {rooms.map((room, i) => (
          <div key={room.id} style={{ background: 'var(--surface)', border: `1.5px solid ${room.is_active ? 'rgba(0,200,150,0.3)' : 'var(--border)'}`, borderRadius: 12, padding: '20px', position: 'relative', overflow: 'hidden' }}>
            <div style={{ position: 'absolute', top: 0, left: 0, width: 3, bottom: 0, background: roomColors[i], borderRadius: '3px 0 0 3px' }} />
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}><div style={{ fontFamily: "'Instrument Serif', serif", fontSize: '1.3rem', color: 'var(--text-1)' }}>Room {room.name}</div><span style={{ fontSize: '0.7rem', fontWeight: 600, padding: '3px 9px', borderRadius: 99, background: room.is_active ? '#d1fae5' : '#fee2e2', color: room.is_active ? '#065f46' : '#991b1b' }}>{room.is_active ? 'ON' : 'OFF'}</span></div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {[{ label: 'Energy Saved', val: `${(room.energy_saved_today || 0).toFixed(3)} kWh` }, { label: 'Peso Savings', val: `₱${((room.energy_saved_today || 0) * ZAMCELCO_RATE).toFixed(2)}` }, { label: 'Status', val: room.occupancy ? 'Occupied' : 'Vacant' }].map((s, j) => (<div key={j} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem' }}><span style={{ color: 'var(--text-3)' }}>{s.label}</span><strong style={{ color: 'var(--text-1)' }}>{s.val}</strong></div>))}
            </div>
          </div>
        ))}
      </div>
      {rooms.length === 0 ? (<div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '48px', textAlign: 'center' }}><div style={{ fontSize: '0.9rem', color: 'var(--text-3)' }}>No rooms found.</div></div>) : (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '24px' }}><div style={{ fontWeight: 600, fontSize: '0.9rem', color: 'var(--text-1)', marginBottom: 4 }}>Energy Saved — All Rooms</div><div style={{ fontSize: '0.75rem', color: 'var(--text-3)', marginBottom: 20 }}>kWh saved per day this week</div><Bar data={{ labels: weekDays, datasets: energyDatasets }} options={chartOpts} /></div>
...
function TimelinePage({ rooms, selectedRoom }) {
  ...
  useEffect(() => {
    if (playing) { intervalRef.current = setInterval(() => { setProgress(p => { if (p >= 100) { setPlaying(false); return 100 } return Math.min(p + (0.5 * speed), 100) }) }, 100) }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '24px', marginBottom: 20 }}>
        <input type="range" min={0} max={100} value={progress} onChange={e => { setProgress(Number(e.target.value)); setPlaying(false) }} style={{ width: '100%', accentColor: 'var(--green)', cursor: 'pointer', marginBottom: 16 }} />
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.75rem', color: 'var(--text-3)', marginBottom: 20 }}><span>{events[0] ? new Date(events[0].timestamp).toLocaleTimeString() : '—'}</span><span style={{ color: 'var(--green)', fontWeight: 500 }}>{Math.round(progress)}% through the day</span><span>{events[events.length - 1] ? new Date(events[events.length - 1].timestamp).toLocaleTimeString() : '—'}</span></div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          <button onClick={() => setPlaying(!playing)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 22px', background: playing ? 'var(--bg)' : 'var(--green)', color: playing ? 'var(--text-1)' : 'white', border: '1.5px solid var(--border)', borderRadius: 9, fontFamily: 'Geist, sans-serif', fontSize: '0.88rem', fontWeight: 500, cursor: 'pointer' }}>{playing ? '⏸ Pause' : '▶ Play'}</button>
          <button onClick={reset} style={{ padding: '9px 16px', background: 'var(--bg)', border: '1.5px solid var(--border)', borderRadius: 9, fontFamily: 'Geist, sans-serif', fontSize: '0.88rem', color: 'var(--text-2)', cursor: 'pointer' }}>↺ Reset</button>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><span style={{ fontSize: '0.78rem', color: 'var(--text-3)' }}>Speed:</span>{[0.5, 1, 2, 4].map(s => (<button key={s} onClick={() => setSpeed(s)} style={{ padding: '5px 10px', borderRadius: 6, fontSize: '0.75rem', fontWeight: 600, border: `1px solid ${speed === s ? 'var(--green)' : 'var(--border)'}`, background: speed === s ? 'var(--green-dim)' : 'var(--bg)', color: speed === s ? 'var(--green)' : 'var(--text-3)', cursor: 'pointer' }}>{s}×</button>))}</div>
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8, padding: '7px 14px', background: lightsOnAtCurrent ? 'rgba(0,200,150,0.1)' : 'rgba(239,68,68,0.08)', border: `1px solid ${lightsOnAtCurrent ? 'rgba(0,200,150,0.3)' : 'rgba(239,68,68,0.2)'}`, borderRadius: 8 }}><span style={{ width: 8, height: 8, borderRadius: '50%', background: lightsOnAtCurrent ? 'var(--green)' : 'var(--red)', display: 'inline-block', animation: lightsOnAtCurrent ? 'livePulse 1.8s infinite' : 'none' }} /><span style={{ fontSize: '0.8rem', fontWeight: 600, color: lightsOnAtCurrent ? 'var(--green)' : 'var(--red)' }}>Room {selectedRoom} — Lights {lightsOnAtCurrent ? 'ON' : 'OFF'}</span></div>
        </div>
      </div>
...
function ReportPage({ rooms, selectedRoom }) {
  ...
  const handleExport = () => {
    ...
    const a = document.createElement('a'); a.href = url; a.download = `sec-room-${selectedRoom}-report.csv`; a.click()
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
      ...
      <div className="report-stat-grid">
        {[
          { icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--green)" strokeWidth="2"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>, bg: 'rgba(0,200,150,0.1)', val: totalEvents, label: 'Total Events' },
          { icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#6366f1" strokeWidth="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>, bg: 'rgba(99,102,241,0.1)', val: `${hoursOn}h`, label: 'Total Hours ON' },
          { icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#f59e0b" strokeWidth="2"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>, bg: 'rgba(245,158,11,0.1)', val: `₱${pesoCost}`, label: 'Peso Savings' },
          { icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#10b981" strokeWidth="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>, bg: 'rgba(16,185,129,0.1)', val: `${occupancyPct}%`, label: 'Occupancy Rate' },
          { icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ef4444" strokeWidth="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>, bg: 'rgba(239,68,68,0.1)', val: energySaved.toFixed(2), label: 'kWh Saved' },
          { icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#8b5cf6" strokeWidth="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>, bg: 'rgba(139,92,246,0.1)', val: onEvents, label: 'Motion Detections' },
        ].map((s, i) => (<div key={i} className="report-stat-card"><div className="report-stat-icon" style={{ background: s.bg }}>{s.icon}</div><div className="report-stat-val">{s.val}</div><div className="report-stat-label">{s.label}</div></div>))}
      </div>
      <div className="charts-row" style={{ marginBottom: 24 }}>
        <div className="chart-panel"><div className="chart-panel-head"><span className="chart-panel-title">Daily Light Hours</span><span className="chart-panel-sub">Hours lights were on per day</span></div><Bar data={{ labels: weekLabels, datasets: [{ data: weekHours, backgroundColor: 'rgba(0,200,150,0.18)', borderColor: 'rgba(0,200,150,0.75)', borderWidth: 1.5, borderRadius: 6 }] }} options={chartOpts} /></div>
      ...
      <div className="device-page">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '28px' }}>
              ...
              {tutorialSteps.map((step) => (
                <div key={step.num} onClick={() => setActiveStep(activeStep === step.num ? null : step.num)} style={{ background: activeStep === step.num ? `${step.color}08` : 'var(--bg)', border: `1.5px solid ${activeStep === step.num ? step.color + '44' : 'var(--border)'}`, borderRadius: 10, padding: '16px', cursor: 'pointer', transition: 'all 0.18s' }}>
                  <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                    ...
                    <div style={{ flex: 1 }}><div style={{ fontSize: '0.9rem', fontWeight: 600, color: 'var(--text-1)', marginBottom: 4 }}>{step.title}</div><div style={{ fontSize: '0.82rem', color: 'var(--text-2)', lineHeight: 1.6 }}>{step.desc}</div>
                      <AnimatePresence>{activeStep === step.num && (<motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} style={{ overflow: 'hidden' }}><div style={{ marginTop: 12, padding: '12px 14px', background: `${step.color}10`, border: `1px solid ${step.color}30`, borderRadius: 8, fontSize: '0.8rem', color: 'var(--text-2)', lineHeight: 1.7 }}>💡 {step.detail}</div></motion.div>)}</AnimatePresence>
                    </div>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 10, padding: '14px 16px', fontSize: '0.82rem', color: '#92400e', lineHeight: 1.6 }}><span style={{ fontSize: '1.1rem', flexShrink: 0 }}>⚠️</span><span><strong>Safety Note:</strong> Your 2.5V 3W bulb uses about 1.2A at full voltage. With 9V battery and 680Ω resistor, current ≈ 9.6mA — well within safe range. Never connect the bulb directly to 9V without the resistor or it will instantly burn out. The relay isolates the high-current bulb circuit from the ESP32 low-voltage logic circuit.</span></div>
          </div>
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '28px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8, flexWrap: 'wrap', gap: 12 }}>
              <div><h3 style={{ fontFamily: "'Instrument Serif', serif", fontSize: '1.3rem', color: 'var(--text-1)', marginBottom: 4 }}>3D Circuit Wiring Model</h3><p style={{ fontSize: '0.82rem', color: 'var(--text-3)', lineHeight: 1.6 }}>Interactive 3D view of your exact components. Drag to orbit · Scroll to zoom · Right-click to pan. Colored wires show data flow direction.</p></div>
              <button onClick={() => setModelMotion(!modelMotion)} style={{ padding: '8px 18px', background: modelMotion ? 'rgba(0,200,150,0.1)' : 'var(--bg)', border: `1.5px solid ${modelMotion ? 'var(--green)' : 'var(--border)'}`, borderRadius: 8, fontFamily: 'Geist, sans-serif', fontSize: '0.82rem', fontWeight: 500, color: modelMotion ? 'var(--green)' : 'var(--text-2)', cursor: 'pointer', transition: 'all 0.18s', display: 'flex', alignItems: 'center', gap: 7 }}><span style={{ width: 8, height: 8, borderRadius: '50%', background: modelMotion ? 'var(--green)' : '#ccc', display: 'inline-block', animation: modelMotion ? 'livePulse 1.8s infinite' : 'none' }} />{modelMotion ? 'Motion Active — Bulb ON' : 'Simulate Motion'}</button>
            </div>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }}>{[{ color: '#ff4444', label: 'VCC / Power' },{ color: '#222222', label: 'GND' },{ color: '#00c896', label: 'PIR Signal (GPIO 13)' },{ color: '#4488ff', label: 'Relay Control (GPIO 2)' },{ color: '#ff6600', label: 'Bulb Circuit (9V)' },{ color: '#ff8844', label: 'Relay Output' }].map((item, i) => (<div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'var(--bg)', borderRadius: 6, padding: '4px 10px', border: '1px solid var(--border)' }}><span style={{ width: 10, height: 3, borderRadius: 2, background: item.color, display: 'inline-block' }} /><span style={{ fontSize: '0.72rem', color: 'var(--text-2)', fontFamily: 'monospace' }}>{item.label}</span></div>))}</div>
            <ESP32WiringModel showMotion={modelMotion} />
            <div style={{ marginTop: 14, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>{[{ title: 'PIR → ESP32', desc: 'GPIO 13 reads HIGH when motion detected', icon: '👁️' },{ title: 'ESP32 → Relay', desc: 'GPIO 2 sends signal to switch relay coil', icon: '⚡' },{ title: 'Relay → Bulb', desc: '9V battery powers 2.5V bulb via relay + resistor', icon: '💡' }].map((info, i) => (<div key={i} style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 8, padding: '12px 14px' }}><div style={{ fontSize: '1rem', marginBottom: 6 }}>{info.icon}</div><div style={{ fontSize: '0.82rem', fontWeight: 600, color: 'var(--text-1)', marginBottom: 4 }}>{info.title}</div><div style={{ fontSize: '0.75rem', color: 'var(--text-3)', lineHeight: 1.5 }}>{info.desc}</div></div>))}</div>
          </div>
        </div>
      </div>
    </motion.div>
    ...
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '24px' }}>
          <div style={{ fontWeight: 600, fontSize: '0.95rem', color: 'var(--text-1)', marginBottom: 16 }}>Project Location</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, fontSize: '0.85rem', color: 'var(--text-2)', lineHeight: 1.7 }}><div><strong style={{ color: 'var(--text-1)' }}>Institution:</strong> Western Mindanao State University</div><div><strong style={{ color: 'var(--text-1)' }}>Address:</strong> Baliwasan Road, Camp B, Zamboanga City, Philippines</div><div><strong style={{ color: 'var(--text-1)' }}>Department:</strong> College of Computing Studies (CCS)</div><div><strong style={{ color: 'var(--text-1)' }}>Building:</strong> CCS Department Building</div></div>
        </div>
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '24px' }}>
          <div style={{ fontWeight: 600, fontSize: '0.95rem', color: 'var(--text-1)', marginBottom: 16 }}>Technical Stack</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            {[{ label: 'Frontend', val: 'React.js 18 + Vite', color: '#00c896' },{ label: 'Backend', val: 'Django 5 + Django REST', color: '#6366f1' },{ label: '3D Engine', val: 'React Three Fiber + Drei', color: '#f59e0b' },{ label: 'Charts', val: 'Chart.js + React-ChartJS', color: '#ef4444' },{ label: 'Animations', val: 'Framer Motion', color: '#8b5cf6' },{ label: 'Calendar', val: 'React-Calendar', color: '#10b981' },{ label: 'Production DB', val: 'PostgreSQL (Render)', color: '#4488ff' },{ label: 'Local DB', val: 'SQLite (Development)', color: '#888' },{ label: 'Frontend Deploy', val: 'Vercel', color: '#000' },{ label: 'Backend Deploy', val: 'Render', color: '#000' },{ label: 'Hardware', val: 'ESP32 Dev Module', color: '#1a5c22' },{ label: 'Sensor', val: 'HC-SR501 PIR + 5V Relay', color: '#cc4444' }].map((item, i) => (<div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 14px', background: 'var(--bg)', borderRadius: 8 }}><div style={{ width: 8, height: 8, borderRadius: '50%', background: item.color, flexShrink: 0 }} /><div><div style={{ fontSize: '0.72rem', color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{item.label}</div><div style={{ fontSize: '0.85rem', fontWeight: 500, color: 'var(--text-1)' }}>{item.val}</div></div></div>))}
          </div>
        </div>
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '24px' }}>
          <div style={{ fontWeight: 600, fontSize: '0.95rem', color: 'var(--text-1)', marginBottom: 16 }}>Energy Provider</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px', background: 'rgba(0,200,150,0.06)', borderRadius: 10, border: '1px solid rgba(0,200,150,0.2)' }}><div style={{ width: 44, height: 44, borderRadius: 10, background: 'var(--green)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 700, fontSize: '0.8rem' }}>ZC</div><div><div style={{ fontWeight: 600, fontSize: '0.9rem', color: 'var(--text-1)' }}>ZAMCELCO</div><div style={{ fontSize: '0.8rem', color: 'var(--text-3)' }}>Zamboanga City Electric Cooperative</div><div style={{ fontSize: '0.78rem', color: 'var(--text-2)', marginTop: 2 }}>Rate: ₱{ZAMCELCO_RATE}/kWh · Location: Zamboanga City</div></div></div>
        </div>
      </div>
    </motion.div>
  )
...
function AboutPage() {
  const developers = [
    { name: 'Angel Garcia', role: 'Lead Developer & Hardware Integration', color: '#00c896' },
    { name: 'Kurt Adlrich Canilang', role: 'Backend Developer & System Architecture', color: '#6366f1' },
    { name: 'John Paul Enriquez', role: 'Frontend Developer & UI/UX Design', color: '#f59e0b' },
  ]
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '28px' }}>
          <div style={{ fontFamily: "'Instrument Serif', serif", fontSize: '1.3rem', color: 'var(--text-1)', marginBottom: 10 }}>Smart Environment Classroom (SEC)</div>
          <p style={{ fontSize: '0.85rem', color: 'var(--text-2)', lineHeight: 1.75, marginBottom: 16 }}>SEC is an IoT-based smart lighting automation system designed for classroom environments. It uses an ESP32 microcontroller with a PIR motion sensor and relay module to automatically control lighting based on room occupancy, reducing energy waste and operational costs.</p>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 16 }}>{[{ label: 'Project Type', val: 'IoT Automation System' },{ label: 'Subject', val: 'Internet of Things (IoT)' },{ label: 'Course', val: 'BSIT 3A' },{ label: 'Program', val: 'Bachelor of Science in Information Technology' },{ label: 'College', val: 'College of Computing Studies' },{ label: 'University', val: 'Western Mindanao State University' }].map((item, i) => (<div key={i} style={{ padding: '10px 12px', background: 'var(--bg)', borderRadius: 8 }}><div style={{ fontSize: '0.7rem', color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 4 }}>{item.label}</div><div style={{ fontSize: '0.82rem', fontWeight: 500, color: 'var(--text-1)' }}>{item.val}</div></div>))}</div>
          <div style={{ fontWeight: 600, fontSize: '0.9rem', color: 'var(--text-1)', marginBottom: 10 }}>System Purpose</div>
          <p style={{ fontSize: '0.82rem', color: 'var(--text-2)', lineHeight: 1.7, marginBottom: 16 }}>The system addresses energy inefficiency in classroom lighting by implementing motion-based automation. When the PIR sensor detects movement, lights turn on automatically. After a timeout period with no motion, lights turn off. This eliminates manual switching and prevents lights from remaining on in vacant rooms.</p>
          <div style={{ fontWeight: 600, fontSize: '0.9rem', color: 'var(--text-1)', marginBottom: 10 }}>Key Features</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>{['Real-time motion detection via ESP32 and PIR sensor','Automatic relay-controlled light switching','Web dashboard with live status monitoring','Energy savings tracking with ZAMCELCO rate calculation','Historical data viewing by date','Multi-room support and comparison','3D interactive wiring visualization','AI-powered energy insights and recommendations','Automation rules engine','Scenario simulation for testing'].map((feat, i) => (<div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: '0.82rem', color: 'var(--text-2)' }}><span style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--green)', flexShrink: 0 }} />{feat}</div>))}</div>
        </div>
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '28px' }}>
          <div style={{ fontWeight: 600, fontSize: '0.95rem', color: 'var(--text-1)', marginBottom: 16 }}>Hardware Components</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 12 }}>{[{ name: 'ESP32 Dev Module', desc: 'Main WiFi-enabled microcontroller', icon: '🔲' },{ name: 'HC-SR501 PIR', desc: 'Passive infrared motion sensor', icon: '👁️' },{ name: '5V Relay Module', desc: 'Electromechanical switch for bulb', icon: '⚡' },{ name: '2.5V Bulb (3W)', desc: 'Low-voltage indicator lamp', icon: '💡' },{ name: '9V Battery', desc: 'Power source for bulb circuit', icon: '🔋' },{ name: '680Ω Resistor', desc: 'Current limiting for bulb protection', icon: '〰️' },{ name: 'Breadboard', desc: 'Prototyping board for connections', icon: '🟫' },{ name: 'Jumper Wires', desc: 'Male-to-male and male-to-female', icon: '🔌' }].map((h, i) => (<div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px', background: 'var(--bg)', borderRadius: 8 }}><span style={{ fontSize: '1.2rem' }}>{h.icon}</span><div><div style={{ fontSize: '0.82rem', fontWeight: 600, color: 'var(--text-1)' }}>{h.name}</div><div style={{ fontSize: '0.72rem', color: 'var(--text-3)' }}>{h.desc}</div></div></div>))}</div>
        </div>
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '28px' }}>
          <div style={{ fontWeight: 600, fontSize: '0.95rem', color: 'var(--text-1)', marginBottom: 16 }}>Developers</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>{developers.map((dev, i) => (<div key={i} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 16px', background: 'var(--bg)', borderRadius: 10, border: '1px solid var(--border)' }}><div style={{ width: 42, height: 42, borderRadius: '50%', background: dev.color, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 700, fontSize: '0.9rem', flexShrink: 0 }}>{dev.name.split(' ').map(n => n[0]).join('')}</div><div><div style={{ fontSize: '0.9rem', fontWeight: 600, color: 'var(--text-1)' }}>{dev.name}</div><div style={{ fontSize: '0.78rem', color: 'var(--text-3)' }}>{dev.role}</div></div></div>))}</div>
        </div>
      </div>
    </motion.div>
  )
...
function DashboardScreen({ onLogout }) {
  ...
  const generateDemoData = () => {
    const now = new Date(); const demoEvents = []; const demoLogs = []; const hours = [2.5, 4.1, 3.2, 5.0, 1.8, 0.5, 3.7]
    for (let i = 6; i >= 0; i--) { const d = new Date(now); d.setDate(d.getDate() - i); const dateStr = d.toISOString().slice(0, 10); const h = hours[6 - i]; const saved = (h * 0.09).toFixed(3); demoLogs.push({ date: dateStr, hours_on: h, energy_saved: parseFloat(saved) }); const eventCount = Math.floor(Math.random() * 4) + 2; for (let j = 0; j < eventCount; j++) { const t = new Date(d); t.setHours(7 + j * 2, Math.floor(Math.random() * 60), 0); demoEvents.push({ id: `demo-${i}-${j}`, timestamp: t.toISOString(), event_type: j % 2 === 0 ? 'Motion Detected' : 'Lights Auto-OFF', status: j % 2 === 0 ? 'on' : 'off', duration: j % 2 === 0 ? '—' : '7s timeout' }) } }
    demoEvents.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp))
    const demoRoom = { id: 1, name: selectedRoom || 'A', is_active: false, occupancy: false, energy_saved_today: parseFloat(demoLogs[demoLogs.length - 1].energy_saved), last_motion: demoEvents[0].timestamp, events: demoEvents, energy_logs: demoLogs }
    setRooms([demoRoom]); setDemoMode(true); localStorage.setItem('sec_demo', 'true'); addToast('Demo mode active — simulated ESP32 data loaded', 'info')
  }

  const exitDemo = () => { setDemoMode(false); localStorage.removeItem('sec_demo'); loadRooms(); addToast('Exited demo mode — connecting to real ESP32', 'success') }
  ...
  return (
    <div className="dashboard-screen">
      ...
      <aside className={`sidebar ${sidebarOpen ? 'sidebar-mobile-open' : ''}`}>
        ...
        <div style={{ marginBottom: 12, padding: '12px', background: demoMode ? 'rgba(0,200,150,0.1)' : 'rgba(255,255,255,0.03)', borderRadius: 10, border: `1px solid ${demoMode ? 'rgba(0,200,150,0.3)' : 'rgba(255,255,255,0.08)'}` }}>
        ...
        <button className="sidebar-logout" onClick={onLogout}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ marginRight: 7 }}><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>Logout</button>
      </aside>

      <div className="shell">
        <header className="topbar">
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <button className="hamburger-btn" onClick={() => setSidebarOpen(!sidebarOpen)}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="3" y1="6" x2="21" y2="6"/><line x1="3" y1="12" x2="21" y2="12"/><line x1="3" y1="18" x2="21" y2="18"/></svg></button>
            <h1 className="topbar-title">{pageTitles[activeNav]}</h1>
          </div>
          <div className="topbar-meta">
            <span className="topbar-clock">{clock}</span>
            <button className="calendar-trigger-btn" onClick={() => setShowCalendar(!showCalendar)}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg></button>
            <select className="room-select" value={selectedRoom} onChange={e => { setSelectedRoom(e.target.value); setViewingDate(null); setHistoricalData(null) }}>{rooms.map(room => <option key={room.id} value={room.name}>Room {room.name}</option>)}</select>
          </div>
        </header>
